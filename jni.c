...
static int set_boolean(struct libctx *ctx, jobject jobj, const char *name, int value)
{
	jclass jcls = (*ctx->jenv)->GetObjectClass(ctx->jenv, jobj);
...
static jobject new_form_choice(struct libctx *ctx, struct oc_choice *choice)
{
	...
	if (set_string(ctx, jobj, "name", choice->name) ||
	    ...
	    set_boolean(ctx, jobj, "selected", choice->selected)) {
		return NULL;
	}
...
JNIEXPORT void JNICALL Java_org_infradead_libopenconnect_LibOpenConnect_forceReconnect(
	JNIEnv *jenv, jobject jobj)
{
	write_cmd_pipe(jenv, jobj, OC_CMD_RECONNECT);
}
